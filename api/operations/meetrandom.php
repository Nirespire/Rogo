...
class RequestObject{
	...
	public function performRequest(){
		...
		try{
			...
			$query = 'SELECT tid AS tip_id, tip FROM meet_random WHERE tid IN ('.implode(',',$randomIDs).') LIMIT :count';
			$tipsStatement = $this->_sqlCon->prepare('SELECT tid AS tip_id, tip FROM meet_random WHERE tid IN ('.implode(',',$randomIDs).') LIMIT :count');
			...
			logError($_SERVER['SCRIPT_NAME'],__LINE__,'Unable to fetch random meeting tips from database',$e->getMessage(),time());
			$this->setResult(STATUS_FAILURE,'Something went wrong while trying to fetch random meeting tips for you! Please try again!');
		}
	}
	...
	private function getMaxTipID(){
		$maxStatement = $this->_sqlCon->query('SELECT MAX(tid) AS maxid FROM meet_random');
